...
**Two easy-to-reintroduce bugs, found and fixed while building this — don't undo either:**
1. `new Producer(...)` and `.connect()` both throw **synchronously** on a bad config value (confirmed locally: a `node-rdkafka` build without OpenSSL support throws `Unsupported value "sasl_ssl" for configuration property "security.protocol"` right out of the constructor). The whole construct-and-wire-up block in `connectKafkaProducer()` is wrapped in one `try/catch` for exactly this reason — if you refactor this and only wrap `.connect()` (as an earlier draft of this file did), a construction-time throw becomes an unhandled rejection out of `connectKafkaProducer()`, which crashes the whole service at startup instead of degrading gracefully. This is exactly the failure mode the "never throws" contract above exists to prevent.
2. `node-rdkafka`'s plain `Producer` **does not automatically process delivery events** — per its own README: "We must either call `.poll()` manually after sending messages or set the producer to poll on an interval (`.setPollInterval`). Without this, we do not get delivery events and the queue will eventually fill up." Confirmed locally: without `kafkaProducer.setPollInterval(POLL_INTERVAL_MS)` (called right after `.connect()`), `'delivery-report'` never fires at all — not "fires slowly," **never**, no matter how long you wait — and `'ready'` still fires fine, so this is easy to miss in casual testing. Don't remove the `setPollInterval` call thinking it's unnecessary boilerplate; without it, every delivery failure goes completely undetected and, at sustained volume, `produce()` starts throwing `ERR__QUEUE_FULL`.
3. **This entire producer → Aiven → consumer path was verified against the real cluster during development** (real SASL_SSL handshake, a real message produced and delivery-confirmed, consumed by `notification/`, and a real email sent) — not just type-checked/unit-tested. If `node-rdkafka` ever needs rebuilding locally on macOS and throws the OpenSSL error above, the fix is `CPPFLAGS=-I$(brew --prefix openssl)/include LDFLAGS=-L$(brew --prefix openssl)/lib npm rebuild node-rdkafka` (per the package's own README) — this is a local macOS build quirk, not a code bug, and the Dockerfile's builder-stage `apk add openssl-dev` avoids it entirely in the actual deployed image.
