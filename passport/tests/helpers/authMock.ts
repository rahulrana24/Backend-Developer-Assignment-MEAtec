...
export function mockAuthVerifyTimeout(): nock.Scope {
  const timeoutMs = Number(process.env.AUTH_VERIFY_TIMEOUT_MS ?? 2000);
  return nock(AUTH_SERVICE_URL)
    .post('/api/auth/verify')
    .delay(timeoutMs + 500)
    .reply(200, { success: true, message: 'Token verified successfully', data: {} });
}
